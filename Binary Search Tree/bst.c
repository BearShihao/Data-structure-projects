...
int bstRange(struct node *t)
{
	...
	struct node *temp_Small = t->left;
	struct node *temp_Big = t->right;

	while (temp_Small != NULL)
	{
		smallest = temp_Small->value;
		temp_Small = temp_Small->left;
	}
	while (temp_Big != NULL)
	{
		biggest = temp_Big->value;
		temp_Big = temp_Big->right;
	}
...
struct node *bstDeleteLeaves(struct node *t)
{
	...
	// If the current node is a leaf, delete it
	if (t->left == NULL && t->right == NULL)
	{
		free(t);
		return NULL;
	...
	// Recursively delete leaves from the left and right subtrees
...
int bstClosest(struct node *t, int val)
{
	if (t == NULL)
	{
		return 0; // If the tree is empty, return a large value (or handle the case as needed)
	}
	...
	while (current != NULL)
	{
		...
		if (abs(current->value - val) < abs(closest - val))
		{
		...
		{
			// If the current node's value is exactly equal to the target value
